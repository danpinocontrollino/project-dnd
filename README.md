...
```
P(win) ≤ min( sigmoid(0.192 · survive_rounds − 2.086 · ln(kill_rounds) + 4.122),  lattice(CR, count, level) )
```

Everything here is **calibrated against external deathmatch simulation**, not hand-tuned. The **race cap** is a binomial-weighted logistic fit of simulated P(win) on both sides of the damage race — rounds the party survives, and rounds it needs to kill the roster (unclipped, so a 10,000-HP wall stays hopeless) — over the Battlecast guard + OOD grids, with the grid cells rebuilt through the same bestiary profiles the app serves. Earlier versions (hand-tuned, then a survival-only Battlecast fit `sigmoid(1.63·s − 3.98)`) were blind to slow attrition losses: one Lich vs a level-5 party survives 4+ estimated rounds, so the old cap sat at 0.95 while the simulator had the party at **0.000** over 2,000 deathmatches. And because two TTK features cannot price a spell rotation, wherever simulated truth exists we serve it directly: the **lattice** trilinearly interpolates the guard grid itself over (CR {2, 5, 10, 15, 21} × count {1, 2, 4, 8, 12, 19} × level {1, 5, 9, 13, 17, 20}), made monotone at build time, abstaining below CR 2 (`battlecast_bridge/guard_lattice.json`). On grid cells the served probability *is* the simulated one. Both caps are monotone in level and count, so every engine guarantee (binary-search validity, count monotonicity, roster dominance) survives. Result: 1 Lich → level **11**, 2 → **16.5**, **3+ → beyond deadly** (4 Liches: 7% at level 20; 8+: 0.0%), matching the simulator's 65% crossings. Pinned by `tests/test_survival_guard.py` and `behavior_suite.py` check 3b.
...
| `battlecast_guard_fit.png` | The damage race in the deathmatch grid + race cap vs the old survival-only guard |
