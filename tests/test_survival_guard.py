...
class TestGuardAnchors:
    ...
    def test_party_deleted_in_one_round_is_hopeless(self):
        assert _cap_sigmoid(1.0) == pytest.approx(0.10, abs=0.01)

    def test_two_round_survival_is_a_coin_flip_ceiling(self):
        assert _cap_sigmoid(2.0) == pytest.approx(0.50, abs=0.01)

    def test_three_round_survival_barely_binds(self):
        assert _cap_sigmoid(3.0) == pytest.approx(0.90, abs=0.01)

    def test_normal_encounters_unaffected(self):
        # rtk >= 5: cap must exceed any probability the model can output.
        assert _cap_sigmoid(5.0) > 0.998
...
def _rows_for(monster, num_monsters, levels, party_size=4):
    return [
        encounter_row(
            ...
            **_party_config(level, party_size,
                            {"healer": 1, "tank": 1, "arcane": 1, "dps": 1}),
        )
        for level in levels
    ]
...
LICH = MonsterProfile(
    cr=21, hp=135, ac=17, size_num=3, stat_sum=193,
    is_legendary=1, magic_res=0, spellcasting=1,
    atk_bonus=12, dpr=45, save_dc=20, burst=100, name="Lich",
)
GOBLIN = MonsterProfile(
    cr=0.25, hp=7, ac=15, size_num=2, stat_sum=61,
    atk_bonus=4, dpr=5, save_dc=13, burst=5, name="Goblin",
)
...
class TestGuardOnEncounters:
    ...
    def test_cap_monotone_decreasing_in_monster_count(self):
        caps = [
            _survival_cap(_rows_for(LICH, n, [12]))[0]
            for n in (1, 2, 4, 8, 16)
        ]
