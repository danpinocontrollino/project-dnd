...
def _row(**overrides):
    base = dict(
        avg_party_level=5.0, party_size=4, num_monsters=1,
        avg_monster_cr=5.0, avg_monster_hp=100.0, avg_monster_ac=15.0,
        has_healer=1, has_tank=1, has_arcane=1, has_martial_dps=1,
        monster_is_legendary=0, monster_has_mobility=0,
        avg_monster_size_num=3.0, avg_monster_stat_sum=150.0,
        monster_has_physical_res=0, monster_immune_to_cc=0,
        monster_has_magic_res=0, monster_has_pack_tactics=0,
        monster_has_spellcasting=0, monster_has_regeneration=0,
    )
...
class TestFeatureEngineer:
    ...
    def test_nan_in_existing_total_columns_filled_from_row(self):
        ...
        row = _row(num_monsters=3, total_monster_hp=np.nan,
                   total_monster_xp=np.nan, max_monster_cr=np.nan,
                   num_monsters_total=np.nan, total_monster_dpr=np.nan,
                   avg_monster_dpr=20.0, max_monster_atk_bonus=6.0,
                   max_monster_save_dc=15.0, max_monster_burst=20.0)
        out = _transform([row])
    ...
    def test_lethality_ratio_sign(self):
        ...
        hard = _transform([_row(avg_monster_cr=24, avg_monster_hp=600,
                                avg_party_level=3)])
...
class TestRosterAggregation:
    ...
    def test_encounter_row_covers_all_raw_inputs(self):
        m = MonsterProfile(cr=5, hp=100, ac=15)
        row = encounter_row(m, avg_party_level=5, party_size=4,
                            num_monsters=1, has_healer=1, has_tank=1,
                            has_arcane=1, has_martial_dps=1)
...
class TestCrPredictorArtifacts:
    def test_json_loader_uses_raw_booster(self):
        ...
        cr = predict_wotc_cr(model, {
            "hp": 135, "ac": 17, "stat_sum": 160, "size_num": 3,
            "is_legendary": 1, "has_mobility": 0, "physical_res": 0,
            "cc_immune": 1, "magic_res": 0, "pack_tactics": 0,
            "spellcasting": 1, "regeneration": 0,
        })
        assert 0.25 <= cr <= 30
    ...
    def test_predict_wotc_cr_is_order_insensitive(self):
        ...
        model = load_cr_predictor()
        feats = {
            "hp": 200, "ac": 18, "stat_sum": 170, "size_num": 5,
            "is_legendary": 1, "has_mobility": 1, "physical_res": 1,
            "cc_immune": 1, "magic_res": 1, "pack_tactics": 0,
            "spellcasting": 0, "regeneration": 0,
        }
