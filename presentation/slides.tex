% ============================================================================
% True Lethality Engine — exam presentation (Statistical Machine Learning)
% Sapienza-styled Beamer deck, 4 presenter parts.
%
% Build:   pdflatex slides.tex   (run twice for total-page numbers)
% Or:      make slides           (from the repo root)
%
% Figures are pulled live from ../figures/ — recompile after `make retrain`
% or `make battlecast` and the deck stays in sync with the artifacts.
% ============================================================================
...
\begin{frame}{Three guards between the model and the user}
  \begin{enumerate}
    ...
    \item \textbf{Survival physics cap} — $P(\text{win}) \leq \sigma(a \cdot \text{TTK} + b)$ where TTK = deterministic rounds-to-kill-the-party. Originally hand-tuned\ldots
  \end{enumerate}
  \begin{exampleblock}{}
...
\begin{frame}{Result 1: the guard, calibrated}
  \begin{columns}[T]
    \column{0.47\textwidth}
      \begin{itemize}
        \item Binomial-weighted logistic fit on the 180 guard cells:
              $\sigma(\textbf{1.63}\,\text{TTK} - \textbf{3.98})$ replaces $\sigma(2.197\,\text{TTK} - 4.394)$.
        \item Survive 1 round $\to$ cap 9\%; 2 rounds $\to$ \textbf{33\%} (was 50\%); 3 $\to$ 71\% (was 90\%); inert above $\approx$3.7 — the hand guess was too generous exactly in the 2–3 round zone.
        \item Lich ladder now: 1$\to$3.25, 2$\to$6.75, 4$\to$12.75, \textbf{8+ $\to$ beyond deadly}.
      \end{itemize}
    \column{0.5\textwidth}
...
\begin{frame}{Result 2: DM mercy, quantified}
  \begin{columns}[T]
    \column{0.47\textwidth}
      \small
      \begin{itemize}
        \item Same encounters through both lenses: our model (table reality) vs Battlecast (deathmatch). Correlation \textbf{0.845}.
        \item The gap \textbf{crosses over}: $\approx$ \textbf{+0.32 above} sim on hard fights (mercy inflation), $\approx$ \textbf{$-$0.14 below} on easy ones (the 83\% ceiling).
        \item Real curated encounters are overwhelmingly deathmatch-easy — table danger is attrition, not hopeless matchups.
      \end{itemize}
      \srcnote{Few hard-fight cells: +0.32 is directional. OOD: extremes agree (HP$\times$20 $\to$ $P\approx0$), mid ranking moderate (Spearman 0.54).}
    \column{0.5\textwidth}
  ...
  \begin{columns}[T]
    ...
    \column{0.42\textwidth}
      ...
      \begin{exampleblock}{Engineering rigor}
        \small 61 unit tests + 13 behavioral axioms (\texttt{make verify}); bootstrap CIs; append-only experiment log; pinned environment; one-command pipeline (\texttt{make retrain}).
      \end{exampleblock}
  \end{columns}
\end{frame}
