...
def main() -> int:
    ...
    check("lich count scaling", all(b >= a for a, b in zip(levels, levels[1:])),
          f"levels={levels}")
    ...
    rows = [
        encounter_row(lich, avg_party_level=l, party_size=4, num_monsters=1,
    ...
    check("curve not a staircase (calibration granularity)",
    ...
    check("guarded curve still monotone in level",
          bool(np.all(np.diff(guarded) >= -1e-9)))
    ...
    check("goblin > ogre > aboleth > tarrasque at level 1",
          all(b <= a + 0.02 for a, b in zip(p1s, p1s[1:])),
          f"p1={[round(x, 2) for x in p1s]}")

    print("5. Boundary verdicts:")
    check("goblin trivial",
          lethality_appraisal(pipe, goblin, 1)["verdict"] == "trivial")
    check("tarrasque beyond_deadly",
    ...
    try:
        ...
        df_cr = pd.DataFrame([{
            "hp": 2000, "ac": 30, "stat_sum": 250, "size_num": 6,
            "is_legendary": 1, "has_mobility": 1, "physical_res": 1,
            "cc_immune": 1, "magic_res": 1, "pack_tactics": 0,
            "spellcasting": 1, "regeneration": 1,
        }])
        god_cr = max(0.25, round(float(crp.predict(df_cr)[0]) * 4) / 4)
        god = MonsterProfile(
            cr=god_cr, hp=10000, ac=50, dpr=999, atk_bonus=99, save_dc=50,
            burst=400, is_legendary=1, regeneration=1, magic_res=1,
            physical_res=1, cc_immune=1, stat_sum=250, size_num=6,
        )
        verdict = lethality_appraisal(pipe, god, 1)["verdict"]
        check("10k-HP god-monster beyond_deadly", verdict == "beyond_deadly",
              f"CR predictor said {god_cr}, verdict={verdict}")
    except FileNotFoundError:
        check("10k-HP god-monster (cr_predictor missing)", False,
